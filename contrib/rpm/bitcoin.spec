...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/xeroxcoind.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/xeroxcoin-cli.1
Source22:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/xeroxcoin-qt.1

#selinux
Source30:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/contrib/rpm/bitcoin.te
# Source31 - what about xeroxcoin-tx and bench_bitcoin ???
...
The xeroxcoin-cli utility allows you to communicate and control a bitcoin daemon
over RPC, the xeroxcoin-tx utility allows you to create a custom transaction, and
...
mv %{buildroot}%{_bindir}/xeroxcoind %{buildroot}%{_sbindir}/xeroxcoind

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/xeroxcoind 0750 bitcoin bitcoin -
EOF
...
PID_FILE="/run/xeroxcoind/xeroxcoind.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/xeroxcoind -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/xeroxcoind.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/xeroxcoin-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/xeroxcoin-qt.1
%endif
...
%attr(0755,root,root) %{_bindir}/xeroxcoin-qt
...
%attr(0644,root,root) %{_mandir}/man1/xeroxcoin-qt.1*
%endif
...
%attr(0755,root,root) %{_sbindir}/xeroxcoind
...
%attr(0644,root,root) %{_mandir}/man1/xeroxcoind.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/xeroxcoin-cli
%attr(0755,root,root) %{_bindir}/xeroxcoin-tx
%attr(0755,root,root) %{_bindir}/bench_bitcoin
%attr(0644,root,root) %{_mandir}/man1/xeroxcoin-cli.1*
