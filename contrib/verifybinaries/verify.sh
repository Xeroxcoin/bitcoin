...
###   xeroxcoin.org and bitcoin.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "bitcoin.org failed to provide signature file, but xeroxcoin.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoin.org and xeroxcoin.org signature files were not equal?"
